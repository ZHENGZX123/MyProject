...
public class MessageType {
    ...
    public final static int SUREREPONSE = OFFSCREENSHARE + 1;//确认是否听懂，是与否在 msg中体现，1听懂，0听不懂
    ...
    public final static int USE_4G = USE_WIFI + 1;//**暂时不用   是否使用移动数据  msg 0不 1使
    ...
    public final static int USE_SOUND = USE_MICROPHONE + 1;//**暂时不用 是否使用声音 msg 0不 1使
    public final static int USE_SYSTEM_UPDATE = USE_SOUND + 1;//是否使用系统更新 msg 0不 1使
    public final static int USE_WIFI_HOTSPOT = USE_SYSTEM_UPDATE + 1;//是否使用便携式热点 msg 0不 1使
    public final static int USE_ADDITIONAL_FUNCTION = USE_WIFI_HOTSPOT + 1;//**暂时不用 是否使用辅助功能 msg 0不 1使
    public final static int SCREEN = USE_ADDITIONAL_FUNCTION + 1;//横屏 msg 0横屏 1竖屏

    public final static int SHARE_FILE = SCREEN + 1;//分享文件

    public final static int SERVER_FIALIT = 0x100;//服务器异常
}
