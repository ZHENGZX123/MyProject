package cn.kiway.mdm.hprose.jrf;
...
public class RemoteInputStream extends InputStream {
	...
	private long sendAction(MsgISAction.StreamAction action, short fileID, long val) throws IOException {
		JRFClient cli = info.cli;
    ...
    public long skip(final long len) throws IOException {
		...
		if (len == 0)
			return 0;
		return sendAction(MsgISAction.StreamAction.SKIP, info.fileID, len);
    }
    ...
    public int available() throws IOException {
		if (info.cli == null)
			throw new IOException("Closed");
		return (int)sendAction(MsgISAction.StreamAction.AVAILABLE, info.fileID, -1l);
	}
    ...
    public boolean markSupported() {
		if (info.cli == null)
			return false;
		try {
			boolean ret = (sendAction(MsgISAction.StreamAction.MARK_SUPPORTED, info.fileID, -1l) != 0l);
			ex = null;
			return ret;
		} catch (IOException e) {
			ex = e;
	...
	public void mark(int readLimit) {
		if (info.cli == null)
			return;
		try {
			sendAction(MsgISAction.StreamAction.MARK, info.fileID, readLimit);
			ex = null;
		} catch (IOException e) {
			ex = e;
		}
	...
	public void reset() {
		if (info.cli == null)
			return;
		try {
			sendAction(MsgISAction.StreamAction.RESET, info.fileID, -1l);
			ex = null;
		} catch (IOException e) {
			ex = e;
		}
