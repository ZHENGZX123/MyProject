...
/***/ function(module, exports, __webpack_require__) {
	...
	__weex_define__('@weex-component/8c0487da64929c04668c2acc90ecfa73', [], function(__weex_require__, __weex_exports__, __weex_module__) {
	...
	__weex_bootstrap__('@weex-component/8c0487da64929c04668c2acc90ecfa73',undefined,undefined)
	      ...
	      "children": [
	        {
	          ...
	          "children": [
	            ...
	            },
	            {
	              ...
	              "attr": {
	                "value": function () {return '建班老师  ' + (this.teacherName)}
	              }
	            }
	          ]
	        },
	        ...
	        },
	        {
	          ...
	          "repeat": function () {return this.classStudents},
	          "events": {
	            "click": function ($event) {this.radioCheck(this.$index,this.id,this.appellation,$event)}
	          },
	          "children": [
	            {
	              "type": "image",
	            ...
	            },
	            {
	              ...
	              "attr": {
	                "value": function () {return this.appellation}
	              }
	            },
	            {
	              "type": "image",
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = {
	  ...
	  created: function created() {
	    ...
	    this.blankRadio = Utils.ip + 'yjpt/images/blank_radio2.png';
	    this.selectRadio = Utils.ip + 'yjpt/images/select_radio2.png';
	    for (var i in this.classStudents) {
	      this.classStudents[i].status = this.blankRadio;
	    }

	    Utils.changeImg(this.classStudents, ['photo']);
	    this.$on('naviBar.leftItem.click', function (e) {
	      Utils.navigate.pop(this, 'true');
	    });

	    var self = this;
	    var className, userId, teacherName, classId;
	    storage.getItem('className', function (e) {
	      self.className = e.data;
	    });
	    storage.getItem('teacherName', function (e) {
	      self.teacherName = e.data;
	    });
	    ...
	    storage.getItem('classId', function (e) {
	      self.classId = e.data;
	      Utils.fetch({
	        url: '/classChildren/searchAll',
	        data: 'classId=' + self.classId,
	        type: 'json',
	        method: 'POST',
	        success: function success(ret) {
	          var classStudents = ret.data;
	          var arr = [];

	          var retdata = eval('(' + classStudents + ')');
	          for (var i in retdata.childrenList) {
	            var obj = retdata.childrenList[i];

	            var data = {
	              id: obj.id,
	              photo: obj.avatar == null ? Utils.ip + 'yjpt/images/IMG_df.png' : Utils.ip + 'yjpt' + obj.avatar,
	              appellation: obj.child_name,

	              status: self.blankRadio
	            };
	            arr.push(data);
	          }
	          self.classStudents = arr;

	          console.log(self.classStudents);
	        }
	      });
	    });
	    ...
	    this.$on('naviBar.rightItem.click', function (e) {
	      if (this.curraId != 0) {
	        modal.alert({
	          ...
	          storage.setItem('childId', self.curraId, function (e) {});
	          storage.setItem('childName', self.appellations, function (e) {});
	          storage.setItem('classId', self.classId, function (e) {});

	          var url = Utils.setOpenUrl(self.$getConfig(), 'wsbbxx');
	  ...
	  methods: {
	    radioCheck: function radioCheck(index, id, childName) {
	      var self = this;
	      Utils.fetch({
	        url: '/classChildren/myChilds',
	        data: 'userId=' + self.userId,
	        method: 'POST',
	        dataType: 'json',
	        success: function success(ret) {
	          function changeStatus(index, id) {
	            if (self.classStudents[index].status == self.blankRadio) {
	              for (var i in self.classStudents) {
	                self.classStudents[i].status = self.blankRadio;
	              }
	              self.classStudents[index].status = self.selectRadio;
	            }
	            self.curraId = id;
	            self.appellations = childName;
	          }
	          var data = ret.data;
	          if (data.childsInfo.length > 0) {
	            for (var i in data.childsInfo) {
	              if (data.childsInfo[i].childId == id) {
	                modal.alert({
	                  message: '您已经绑定了您的宝贝',
	                  okTitle: '好的'
	                }, function () {
