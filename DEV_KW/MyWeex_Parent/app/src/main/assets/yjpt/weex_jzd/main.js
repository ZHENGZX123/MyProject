	  ...
	  "children": [
	    {
	      ...
	      "children": [
	          ...
	          "children": [
	            {
	              ...
	              "events": {
	                "click": function ($event) {this.openTypePage(this.url,$event)}
	              },
	              "children": [
	                {
	                  "type": "image",
	        ...
	        {
	          "type": "div",
	          "shown": function () {return this.studentList.length>0},
	          "children": [
	            {
	              "type": "div",
	              "classList": [
	              ...
	              "children": [
	                {
	                  ...
	                  "children": [
	                    ...
	                    },
	                    {
	                      ...
	                      "events": {
	                        "click": function ($event) {this.joinClass('showMenu',$event)}
	                      },
	                      "attr": {
	                        "value": "扫码加班"
	                      }
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = {
	    data: function () {return {
	        ...
	        picAdd: '',
	        isShowClass: '0',
	        menuList: [{ pictureUrl: 'yjpt/images/pic_zt01.png', title: '智慧课堂', url: 'catalog-list' }, { pictureUrl: 'yjpt/images/pic_zt02.png', title: '早教频道', url: 'index_zjpd' }, { pictureUrl: 'yjpt/images/pic_zt03.png', title: '照片发布', url: 'wdxc_fbwz' }],
	        dataList: [{ title: '小一班班群', pictureUrl: 'yjpt/images/Photo_04.png', state: 5, news_content: '你家孩子最近上课开小差', date: '07/01', size: 1, url: 'chat-item', classId: '{{classId}}' }, { title: '小一班讨论组', pictureUrl: 'yjpt/images/Photo_05.png', state: 5, news_content: '明天带两个孩子出去', date: '01/11', size: 1, url: 'chat-item', classId: '{{classId}}' }],
	        ...
	        classNames: [],
	        chooseClass: 'class-list2'
	    }},
	    created: function created() {
	        var self = this;
	        self.lb = Utils.ip + Utils.dir + '/' + 'yjpt/images/banner_01.jpg';
	        self.bjUrl = Utils.ip + Utils.dir + '/' + 'yjpt/images/Photo_bj.png';
	        self.portrait = Utils.ip + Utils.dir + '/' + 'yjpt/images/photo_06.jpeg';
	        self.picAdd = Utils.ip + Utils.dir + '/' + 'yjpt/images/add.png';
	        self.chooseClass = Utils.setOpenUrl(self.$getConfig(), self.chooseClass);

	        Utils.changeImg(self.menuList, ['pictureUrl'], 'menuList');

	        Utils.changeImg(self.dataList, ['pictureUrl']);
	        ...
	        storage.getItem('classMap', function (e) {
	            if (e.data != 'undefined') {
	                self.classMap = JSON.parse(e.data);
	            }
	        });
	        ...
	        storage.getItem('studentList', function (e) {
	            if (e.data != 'undefined') {
	                ...
	                storage.getItem('classMap', function (e) {
	                    if (e.data != 'undefined') {
	                        self.classMap = JSON.parse(e.data);
	                        for (var i in self.classMap) {
	                            for (var j in self.studentList) {
	                                if (self.classMap[i].id == self.studentList[j].classId) {
	                                    self.classNames.push((0, _assign2.default)(self.classMap[i], self.studentList[j]));
	                                }
	                            }
	                        }
	                        ...
	                        storage.setItem('classNames', (0, _stringify2.default)(self.classNames), function () {});
	                    }
	                });
	            }
	        });

	        var bindChild = new BroadcastChannel('bindChild');
	        bindChild.onmessage = function (e) {
	            if (e.data == 'bindChild') {
	                Utils.fetch({
	                    url: '/app/parent/child?userId=' + self.userId,
	                    method: 'get',
	                    dataType: 'json',
	                    success: function success(res) {
	                        if (res.data.StatusCode == '200') {
	                            self.studentList = res.data.data.studentList;
	                            self.classMap = res.data.data.classMap;

	                            for (var i in self.studentList) {
	                                console.log(self.studentList[i].classId);
	                                for (var j in self.classMap) {
	                                    if (!self.classMap[self.studentList[i].classId]) {
	                                        self.studentList.splice(i, 1);
	                                    }
	                                }
	                            }

	                            storage.setItem('studentList', (0, _stringify2.default)(self.studentList), function () {});
	                            storage.setItem('classMap', (0, _stringify2.default)(self.classMap), function () {});
	                        }
	                    }
	                });
	            }
	    ...
	    methods: {
	        openPage: function openPage() {
	            var url = Utils.setOpenUrl(this.$getConfig(), 'sx_xq');
	        ...
	        gotoPage: function gotoPage(classId) {
	            var self = this;
	        ...
	        joinClass: function joinClass(showMenu) {
	            var self = this;
	                ...
	                event.QRScan('', function (result) {
	                    if (result.path) {
	                        Utils.fetch({
	                            ...
	                            dataType: 'json',
	                            success: function success(res) {
	                                if (res.data.StatusCode == '200') {} else {
	                                    modal.toast({
	                                        message: '操作失败，请稍后重试'
	                                    });
	                                }
	        ...
	        joinClassByPhone: function joinClassByPhone() {
	            var self = this;
	            var url = Utils.setOpenUrl(self.$getConfig(), 'join-class-phone');
	            Utils.navigate.push(self, url, 'true');
	        },
	        toggleClass: function toggleClass() {
	            this.isShowClass = '0';
	        },
	        createGroup: function createGroup(url) {
	            var self = this;
	            self.isShowMenu = !self.isShowMenu;
	            Utils.navigate.push(self, url, 'true');
	        },
	        openTypePage: function openTypePage(url) {
	            var self = this;
	            if (self.studentList.length <= 0) {
	                modal.toast({
	                    message: '请先加入班级'
	            ...
	            if (url.indexOf('index_zjpd') == -1) {
	                Utils.navigate.push(this, url, 'true');
	            } else {
	                this.$dispatch('isShowClass', '1');
	            }
	        }
	    }
	};}
