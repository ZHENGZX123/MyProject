...
/***/ function(module, exports, __webpack_require__) {
	...
	__weex_define__('@weex-component/f172fc279502802083bc16758f349217', [], function(__weex_require__, __weex_exports__, __weex_module__) {
	...
	__weex_bootstrap__('@weex-component/f172fc279502802083bc16758f349217',undefined,undefined)
	  ...
	  "children": [
	    {
	      ...
	      "events": {
	        "click": function ($event) {this.openNewsPage(this.id,$event)}
	      },
	      "children": [
	        {
	          "type": "text",
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = {
	  ...
	  methods: {
	    openNewsPage: function openNewsPage(id) {
	      storage.setItem('id', id, function (e) {});

	      var url = Utils.setOpenUrl(this.$getConfig(), 'xxxw_xq');
...
/***/ function(module, exports, __webpack_require__) {
	...
	__weex_define__('@weex-component/new-list', [], function(__weex_require__, __weex_exports__, __weex_module__) {
	              ...
	              "children": [
	                {
	                  "type": "div",
	                  "events": {
	                    "click": function ($event) {this.openNews(this.id,$event)}
	                  },
	                  "children": [
	                    {
	                      "type": "image",
	                    ...
	                    },
	                    {
	                      ...
	                      "attr": {
	                        "value": function () {return this.courseTitle}
	                      }
	                    }
	                  ]
	                },
	...
	module.exports = {
	  ...
	  created: function created() {
	    ...
	    for (var i in this.itemsList) {
	      data = this.itemsList[i];
	      changeData(context, data);
	      if (data.itemList.length > 0) {
	        var dataNews = data.itemList;
	        changeData(context, dataNews);
	      }
	    }
	    this.headLeft = Utils.ip + 'yjpt/images/id_right_bg.png';
	    Utils.changeImg(this.itemsList, ['src', 'pictureUrl'], 'itemList');
	    ...
	    storage.getItem('classId', function (e) {
	      ...
	      Utils.fetch({
	        url: '/news/jsnews',
	        data: 'classId=' + self.classId + '&date=first',
	        dataType: 'json',
	        method: 'POST',
	        success: function success(ret) {
	          ...
	          var newsList = retdata.newsJList;
	          self.day = retdata.day;
	          if (newsList.length > 0) {
	            for (var j in newsList) {
	              dateArr.push(newsList[j].create_date.substring(0, 10));
	            }
	            dateArr = Utils.unique(dateArr);
	            for (var k in dateArr) {
	              ...
	              var innerDataArr = [];
	              for (var l = 0; l < newsList.length; l++) {
	                if (dateArr[k] == newsList[l].create_date.substring(0, 10)) {
	                  innerDataArr.push(newsList[l]);
	                }
	              }

	              if (innerDataArr.length > 0) {

	                var date = new Date(innerDataArr[0].create_date.replace(/\-/g, '/'));
	                var year = date.getFullYear();
	                var month = date.getMonth() + 1;
	                var date1 = date.getDate();
	                var createtime = year + "年" + month + "月" + date1 + "日";

	                itemObj.date = createtime;
	                itemObj.courseTitle = innerDataArr[0].title;
	                itemObj.src = innerDataArr[0].img_path == null || innerDataArr[0].img_path == 'null' ? Utils.ip + 'yjpt/images/5387ff717a670.jpg' : Utils.ip + 'yjpt' + innerDataArr[0].img_path;
	                itemObj.id = innerDataArr[0].id;
	                innerDataArr.splice(0, 1);
	                for (var m in innerDataArr) {
	                  var obj = innerDataArr[m];
	                  var data = {
	                    title: obj.title,
	                    pictureUrl: obj.img_path == null || obj.img_path == 'null' ? Utils.ip + 'yjpt/images/5387ff717a670.jpg' : Utils.ip + 'yjpt' + obj.img_path,
	                    id: obj.id
	                  };
	                  itemObj.itemList.push(data);
	                }
	                outerArr.push(itemObj);
	              }
	            }
	          }
	          self.itemsList = outerArr;
	        }
	  ...
	  methods: {
	    ...
	    handleLoading: function handleLoading(e) {
	      ...
	      if (!self.flag) {
	        Utils.fetch({
	          url: '/news/jsnews',
	          data: 'classId=' + self.classId + '&date=' + self.day,
	          dataType: 'json',
	          method: 'POST',
	          success: function success(ret) {
	            var retdata = ret.data;
	            ...
	            if (newsList.length > 0) {
	              ...
	              for (var k in dateArr) {
	                ...
	                if (innerDataArr.length > 0) {
	                  ...
	                  itemObj.date = createtime;
	                  itemObj.courseTitle = innerDataArr[0].title;
	                  itemObj.src = innerDataArr[0].img_path == null ? Utils.ip + 'yjpt/images/5387ff717a670.jpg' : Utils.ip + 'yjpt' + innerDataArr[0].img_path;
	                  itemObj.id = innerDataArr[0].id;
	                  innerDataArr.splice(0, 1);
	                  for (var m in innerDataArr) {
	                    var obj = innerDataArr[m];
	                    var data = {
	                      title: obj.title,
	                      pictureUrl: obj.img_path == null ? Utils.ip + 'yjpt/images/5387ff717a670.jpg' : Utils.ip + 'yjpt' + obj.img_path,
	                      id: obj.id
	                    };
	                    itemObj.itemList.push(data);
	                  }
	            ...
	            outerArr.forEach(function (arr, index) {
	              self.itemsList.push(arr);
	            });
	          }
	        });
	      } else {
