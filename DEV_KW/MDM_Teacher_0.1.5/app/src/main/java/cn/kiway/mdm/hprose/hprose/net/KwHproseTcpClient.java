...
abstract class KwSocketTransporter implements KwConnectionHandler {
    protected final static class ConnectorHolder {
        ...
        public static final void create(String uri, KwConnectionHandler handler, boolean keepAlive, boolean noDelay) throws IOException {
...
final class KwFullDuplexSocketTransporter extends KwSocketTransporter {
    ...
    private final Map<KwConnection, Map<Integer, Response>> responses = new ConcurrentHashMap<KwConnection, Map<Integer, Response>>();
...
public class KwHproseTcpClient extends HproseClient {
	...
	public long getFile(String local, String remote){
		...
		try {
			...
			len=cli.getFile(remote, 3, local, 1500);
//			byte[] buf = new byte[5120];
//			OutputStream os = new BufferedOutputStream(new FileOutputStream(local));
//			try (InputStream is = cli.getRemoteInputStream(remote, deflate)) {
//				int n;
//				for (;;) {
//					n = is.read(buf);
//					if (n < 0){
...
//			os.close();
    ...
    private final KwSocketTransporter fdTrans = new KwFullDuplexSocketTransporter(this);
    private final KwSocketTransporter hdTrans = new KwHalfDuplexSocketTransporter(this);

    public KwHproseTcpClient() {
        super();
    }

    public KwHproseTcpClient(String uri) {
        super(uri);
        this.uri = uri;
    }
