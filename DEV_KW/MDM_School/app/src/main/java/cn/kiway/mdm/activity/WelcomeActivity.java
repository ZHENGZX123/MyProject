...
public class WelcomeActivity extends BaseActivity {
            ...
            public void run() {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Log.d("test", "删除旧包");
                        if (new File(WXApplication.ROOT + "mdm_school").exists()) {
                            FileUtils.delFolder(WXApplication.ROOT + "mdm_school");
                        }
                        try {
                            Log.d("test", "解压新包");
    ...
    protected void checkIsFirst() {
        ...
        if (getSharedPreferences("kiway", 0).getBoolean("isFirst", true)) {
            ...
            //拷贝
            FileUtils.copyRawToSdcard(this, R.raw.mdm_school, WXApplication.ZIP);
            //解压
            try {
                new ZipFile(WXApplication.ROOT + WXApplication.ZIP).extractAll(WXApplication.ROOT);
            } catch (ZipException e) {
