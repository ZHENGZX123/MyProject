...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = {
	    ...
	    created: function created() {
	        ...
	        storage.getItem('pageTitle', function (e) {
	            if (e.data != 'undefined') {
	                self.pageTitle = e.data;
	            }
	        });
	            ...
	            if (e.data != 'undefined') {
	                ...
	                Utils.fetch({
	                    ...
	                    success: function success(ret) {
	                        self.lessonList = ret.data.courseList;

	                        var date, year, month, day, hour, min, second, currentDate;
	                        date = new Date();
	                        ...
	                        for (var i = 0; i < self.lessonList.length; i++) {
	                            self.lessonList[i]['isFirst'] = false;
	                            if (i < self.lessonList.length - 1) {
	                                if (self.lessonList[i]['attend_date'] == self.lessonList[i + 1]['attend_date']) {
	                                    self.lessonList[i]['isFirst'] = true;
	                                }
	                            }

	                            if (self.lessonList[self.lessonList.length - 1]['attend_date'] != self.lessonList[self.lessonList.length - 2]['attend_date']) {
	                                self.lessonList[self.lessonList.length - 1]['isFirst'] = true;
	                            }

	                            self.setDirType(i);
	                            self.setPhoto(i);

	                            self.lessonList[i]['latestIndex'] = i;
	                            if (self.lessonList[i]['finish_date'] < currentDate) {
	                                latest = self.lessonList[i]['finish_date'];
	                                self.latestIndex = i;
	                            }
	                        }

	                        storage.setItem('lessonList', (0, _stringify2.default)(ret.data.courseList), function () {});
	                        storage.setItem('latestIndex', self.latestIndex, function () {});
	                    }
	                });
	            }
	        });
